...
func getSimulateFromSeedInput(tb testing.TB, app *ChatApp) (
...
func testAndRunTxs(app *ChatApp) []simulation.WeightedOperation {
...
func invariants(app *ChatApp) []sdk.Invariant {
...
// Profile with:
// /usr/local/go/bin/go test -benchmem -run=^$ github.com/openchatproject/openchat/app -bench ^BenchmarkFullChatSimulation$ -SimulationCommit=true -cpuprofile cpu.out
func BenchmarkFullChatSimulation(b *testing.B) {
	// Setup Chat application
	logger := log.NewNopLogger()

	var db dbm.DB
	dir, _ := ioutil.TempDir("", "goleveldb-chat-sim")
	db, _ = sdk.NewLevelDB("Simulation", dir)
	defer func() {
		db.Close()
		os.RemoveAll(dir)
	}()
	app := NewChatApp(logger, db, nil, true, 0)
...
func TestFullChatSimulation(t *testing.T) {
	if !enabled {
		t.Skip("Skipping Chat simulation")
	}
	...
	app := NewChatApp(logger, db, nil, true, 0, fauxMerkleModeOpt)
	require.Equal(t, "ChatApp", app.Name())
...
func TestChatImportExport(t *testing.T) {
	if !enabled {
		t.Skip("Skipping Chat import/export simulation")
	}
	...
	newDir, _ := ioutil.TempDir("", "goleveldb-chat-sim-2")
	newDB, _ := sdk.NewLevelDB("Simulation-2", dir)
	defer func() {
		newDB.Close()
		os.RemoveAll(newDir)
	}()
	newApp := NewChatApp(log.NewNopLogger(), newDB, nil, true, 0, fauxMerkleModeOpt)
	require.Equal(t, "ChatApp", newApp.Name())
...
func TestChatSimulationAfterImport(t *testing.T) {
	if !enabled {
		t.Skip("Skipping Chat simulation after import")
	}
...
// and doesn't depend on openchat
